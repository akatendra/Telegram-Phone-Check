...
def get_names(customer_id, phone_number):
    try:
        ...
        database.telegram_off(customer_id)
        return logger.debug(
        ...
        logger.debug(
            f'FloodWaitError: {error.message} | seconds: {sleep_time}')
...
if __name__ == '__main__':
    ...
    customers = database.get_customers_to_check()
    # customers = list(customers)
    # customers.append((90000, '380669999999'))
    all_count = len(customers)
    ...
    for customer in customers:
        ...
        sleep_time(5)
        api_res = get_names(customer_id, telephone)
        if api_res:
            count += 1
            logger.debug(f'count: {count}')
            chat_id = api_res[0]
            username = api_res[1]
            first_name = api_res[2]
            last_name = api_res[3]
            database.put_telegram_user(customer_id, telephone, chat_id,
                                       username, first_name, last_name)
    percent = count / all_count * 100
    logger.debug(
        f'all_count: {all_count} | count: {count} | have telegram: {percent} %')
